...
[![CI](https://github.com/gwenlake/okflib/actions/workflows/ci.yml/badge.svg)](https://github.com/gwenlake/okflib/actions)
[![PyPI](https://img.shields.io/pypi/v/okflib.svg)](https://pypi.org/project/okflib/)
[![Python](https://img.shields.io/pypi/pyversions/okflib.svg)](https://pypi.org/project/okflib/)
...
No SDK, no runtime, no lock-in. The format is the contract. `okflib` gives you the tooling on top: a clean Python API, a CLI, trust-signal support (OKF v0.2), and LLM-powered ingestion.
...
- **See the graph** — `okflib view` opens an interactive, self-contained HTML page (force-directed, clustered by directory, searchable); plus DOT, Mermaid (renders natively on GitHub) and JSON exports, with broken-link targets shown as ghost nodes.
- **LLM ingestion** — send raw text *or a whole directory of documents* (md, txt, html, csv, json, code; PDF/Word via `okflib[docs]`), get well-formed, cross-linked OKF concepts back with per-file provenance. Anthropic API by default; any provider via a one-function plug.
- **Retrieval** — BM25 ranking (pure Python, accent-insensitive, frontmatter-weighted) for short textual queries, plus `okflib ask`: retrieve-then-answer over your own knowledge.
...
- **A friendly CLI** — `okflib init | add | list | show | verify | validate | search | query | ask | stale | graph | view | stats | log | export | ingest | mcp`. It works on the bundle you are inside, like `git`: `cd` once, then no path to repeat.
...
```bash
uv add okflib          # as a project dependency
uvx okflib --help      # or run the CLI without installing
```
...
`okflib.readers` imports them **lazily**, inside `read_text()`, only when such a file shows
up. Without the extra nothing crashes — the file is simply reported and skipped, and the
run continues:

```
mon_rapport.pdf -> skipped: mon_rapport.pdf: PDF support requires `pip install okflib[docs]`
```
...
```bash
uv add "okflib[docs]"                        # in your project
uvx --from "okflib[docs]" okflib ingest ~/docs  # one-shot, nothing installed
pip install "okflib[docs]"                   # with pip
```
...
```bash
uv run --extra docs okflib ingest ~/docs
```
...
Point `okflib` at a directory and let the LLM do the reading. Three commands, no Python:

```bash
export ANTHROPIC_API_KEY=sk-ant-...

okflib init kb && cd kb     # 1. create a bundle and step into it
okflib ingest ~/docs        # 2. read every document, write OKF concepts
okflib view                 # 3. look at the result in your browser
```
...
```
$ okflib ingest ~/docs --prefix docs
Ingesting docs with claude-sonnet-5
  [1/3] finance/2026_budget.pdf ⚠ skipped: PDF support requires `pip install okflib[docs]`
...
- **Provenance for free** — each concept records its originating file in `sources`, is stamped `generated: {by: okflib/<model>}`, and starts **unverified**.
...
```bash
okflib ingest ~/docs --model claude-haiku-4-5   # per run
export OKFLIB_MODEL=claude-haiku-4-5            # for every command in this shell
```

| Model | Input / output per Mtok | Good for |
|---|---|---|
| `claude-haiku-4-5` | $1 / $5 | Bulk ingestion — extraction and structuring at 3× less than Sonnet |
| `claude-sonnet-5` (default) | $3 / $15 | Denser or more ambiguous sources, and `okflib ask` |

The same flag works on `okflib ask`, where the model reasons over the retrieved concepts
...
```bash
okflib query "oncall escalation"                # BM25 ranking, instant, no LLM
okflib ask "who do I page after 20 minutes?"    # RAG answer with cited concept IDs
okflib list --tier unverified                   # your review queue
okflib verify docs/oncall-rotation --by human:you@example.com
okflib validate                                 # CI-friendly conformance check
okflib view                                     # interactive graph in your browser
```
...
```bash
okflib add notes/idea --type Note --title "Idea" -d "Worth exploring." --tag inbox
okflib list --tier human-reviewed --status stable
okflib show notes/idea
okflib verify notes/idea --by human:you@example.com
okflib validate                # exit code 1 if non-conformant → CI-friendly
okflib search "exploring"
okflib stale --on 2027-01-01   # what needs re-verification by then?
okflib stats                   # counts by type / tier / status, broken links
okflib view                    # interactive HTML graph in the browser
okflib graph --format mermaid  # or dot | json | html
okflib export kb.tar.gz
okflib ingest ./docs           # directory, file, or '-' -> knowledge (LLM)
okflib query "short query"     # BM25 retrieval, no LLM
okflib ask "a question"        # RAG over the bundle
okflib mcp                     # serve to Claude via MCP
```

`okflib init` accepts a directory (`okflib init kb`, or nothing for the current one).
Anything else takes `-C PATH` to work on a bundle you are not inside — `-C` goes before
or after the subcommand, so both `okflib -C kb stats` and `okflib stats -C kb` work.

## See the whole graph: `okflib view`

`okflib view` writes `graph.html` at the bundle root and opens it. It is a single
...
```bash
okflib view                          # write + open
okflib view -o /tmp/kb.html --no-open  # just write it (CI, a static site, an email)
```

For a graph you can paste into a GitHub README or PR description, use Mermaid instead:

```bash
okflib graph --format mermaid
```
...
```bash
okflib query "alerte fraicheur"        # ranked concepts + snippets (no LLM, instant)
okflib ask "comment est calculé le CA ?"   # RAG: retrieve top concepts, LLM answers, sources cited
```

```python
from okflib.search import Index
from okflib.llm import ask
...
```bash
# Claude Code
claude mcp add my-kb -- okflib -C /path/to/kb mcp

# Claude Desktop (mcpServers config)
{"mcpServers": {"my-kb": {"command": "okflib", "args": ["-C", "/path/to/kb", "mcp"]}}}
```
...
1. Generated concepts are stamped `generated: {by: okflib/<model>, at: ...}` and start **unverified** — review them, then `okflib verify` to raise the tier.
...
```python
from okflib import Bundle
from okflib.llm import ingest_dir, ingest_text
...
| `okflib.graph` | `to_html` (interactive page), `to_dot`, `to_mermaid`, `to_json` |
| discovery | `find_bundle_root()` / `is_bundle_root()` — how the CLI finds the bundle you are in |
| `okflib.llm` | `ingest_text`, `ingest_dir`, `ask`, `anthropic_complete`, pluggable `Completer` |
| `okflib.search` | `Index` (BM25), `Hit` |
| `okflib.readers` | `read_text`, `iter_documents` (md, txt, html, csv, json, code; PDF/docx via `[docs]`) |
| `okflib.mcp` | `MCPServer` — stdio MCP server for Claude and other clients |
| `Stamp`, `Source` | v0.2 `{by, at}` stamps and provenance entries with credibility signals |

## Development

```bash
git clone https://github.com/gwenlake/okflib && cd okflib
uv sync --dev
uv run pytest tests/ -q
uv run ruff check okflib tests
uv build
```
...
`okflib` is developed and maintained by [Gwenlake](https://gwenlake.com). Issues and pull
requests are welcome on [github.com/gwenlake/okflib](https://github.com/gwenlake/okflib).
