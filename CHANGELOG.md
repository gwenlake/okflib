...
- `pyokf init` then `cd`, and every command applies to that bundle from anywhere inside
...
- The old form prints a migration hint instead of failing obscurely:
  `pyokf list kb` → `cd kb && pyokf list` or `pyokf -C kb list`.
- `pyokf init` takes an optional directory (defaults to the current one) and is idempotent.
- `ingest` and `ingest-dir` are one command: `pyokf ingest <directory | file | ->`
...
- `ingest` reports progress as it runs — one line per file (`[3/31] paper.pdf ✓ 5 concepts`),
  ...
  under `NO_COLOR`.
- `pyokf view` now shows the **folder hierarchy** alongside the link graph: a hollow node
...
- `pyokf view`: dragging a node keeps the layout live, so neighbours rearrange while you
  hold it and the graph re-settles on release.
- Fixed: `pyokf view` produced a blank page — `hideTip` was a `const` referenced by code
...
- Model selection: `--model` on `ingest` / `ask` (already present) is now joined by the
  `PYOKF_MODEL` environment variable, and the default moves from `claude-sonnet-4-6` to
  `claude-sonnet-5`. `claude-haiku-4-5` is a good, cheaper fit for bulk ingestion.
- Fixed: `pyokf ingest <single file>` read the file as UTF-8 text instead of going through
  `pyokf.readers`, so a lone PDF or .docx crashed with a `UnicodeDecodeError` while the
  same file inside an ingested directory worked.
- `pyokf view`: an interactive HTML knowledge graph, opened in your browser —
  ...
  (no CDN, no dependency); also available as `pyokf graph --format html`.

## 0.4.0

- `pyokf ingest-dir`: ingest every document in a directory (md, txt, html, csv, json, code natively; PDF/Word via the `docs` extra), one LLM call per file, originating file recorded in v0.2 `sources`
- BM25 retrieval (`pyokf.search.Index`, `pyokf query`): pure Python, accent-insensitive, frontmatter-weighted
- `pyokf ask`: retrieve-then-answer (RAG) over the bundle with cited concept IDs
- MCP server (`pyokf mcp`): stdio Model Context Protocol server exposing `search_knowledge` / `read_concept` / `list_concepts` to Claude and other MCP clients
